...
Every ecosystem has a vulnerability scanner — use yours in CI (npm shown):

```bash
# Audit dependencies for known vulnerabilities
npm audit                          # Node/npm  (pnpm audit, yarn npm audit)
# pip-audit                        # Python
# bundle audit                     # Ruby
# cargo audit                      # Rust
# govulncheck ./...                # Go
# osv-scanner / Trivy / Snyk       # language-agnostic

# Auto-fix and gate on severity (npm example)
npm audit fix
npm audit --audit-level=critical
```

**Supply-chain hygiene** (audits won't catch malicious packages):
- [ ] Lockfile committed; CI does a clean, locked install — never an unpinned resolve (`npm ci`, `pnpm install --frozen-lockfile`, `yarn install --immutable`, `pip install --require-hashes`, `bundle install --frozen`)
- [ ] New dependencies reviewed (maintenance, downloads, install/build hooks like npm `postinstall` or pip `setup.py`)
- [ ] No typosquats — verify the exact package name (`cross-env` vs `crossenv`)
...
| 6   | Vulnerable Components     | Audit deps (your ecosystem's scanner), keep updated, minimal deps |
