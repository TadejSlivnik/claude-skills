...
Most items here are framework-agnostic — they rest on web-platform fundamentals (Core Web Vitals, HTML/CSS, browser scheduling APIs, network, fonts, bfcache) that apply no matter what you build with. Where an item names a framework-specific API or library, it's an *example*; the parentheticals list equivalents in other ecosystems. Use whatever your stack provides.
...
- [ ] Expensive components skip re-renders when inputs are unchanged (`React.memo`, Vue `v-memo`/`computed`, Angular `OnPush`, Svelte/Solid's compiled fine-grained reactivity)
- [ ] Memoized computations/derived values used only where profiling shows benefit (`useMemo`/`useCallback`, Vue `computed`, Angular `computed` signals, Solid `createMemo`)
...
- [ ] Long lists use windowing/virtualization (`react-window`/`@tanstack/virtual`, `vue-virtual-scroller`, Angular CDK `cdk-virtual-scroll`)
- [ ] No unnecessary wide re-renders (scope reactivity/component boundaries so a small change doesn't re-render the whole tree)
